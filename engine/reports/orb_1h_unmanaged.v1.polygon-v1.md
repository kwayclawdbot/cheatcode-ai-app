# orb_1h_unmanaged.v1 — measured on `polygon-v1`

**BASKET(32): FAIL.** Against the bar in [`../models/orb_1h_unmanaged.v1/GATE.md`](../models/orb_1h_unmanaged.v1/GATE.md), committed at `d8e592b` before this evaluation existed. one change: **no management** — no partial at +1R, the stop never moves.

**11,591 BASKET(32) trades, 2023-09-11 → 2026-08-28** — 9,041 in-sample and 2,550 in the held-back window.
**Before costs the model made -0.032R a trade against the matched coin flip's -0.033R; paired trade for trade the gap is +0.001R (95%: -0.028 to +0.030, n=11,568).**
**After costs the average trade returned -0.201R and the MIDDLE trade -1.008R.** Realised stop width: median 63.0¢, mean 111.1¢. Cost drag 0.162R.

Run 2026-08-29T21:27:23+00:00 at `d8e592b`. Snapshot `polygon-v1`, commission $0.005/share/side, slippage 1.0bp on market and stop fills.
...
|---|---|---|---|---|---|---|---|---|
| BASKET(32) | `orb_1h_unmanaged.v1` | 11,591 | -0.032 | -0.784 | -0.201 | -1.008 | 44.8% | 0.67 |
| BASKET(32) | `null_coinflip.v1.matched` | 11,590 | -0.033 | -0.709 | -0.197 | -1.006 | 44.9% | 0.67 |
...
|---|---|---|---|---|---|
| BASKET(32) | all | 11,568 | +0.001 | -0.028 to +0.030 | contains zero — nothing measurable |
| BASKET(32) | in-sample | 9,023 | -0.005 | -0.038 to +0.028 | contains zero — nothing measurable |
| BASKET(32) | out-of-sample | 2,545 | +0.021 | -0.038 to +0.080 | contains zero — nothing measurable |
...
|---|---|
| what the setup earned, before costs | -4.50¢ |
| what it paid to get in and out | -5.20¢ |
| **what was left, on the average trade** | **-9.70¢** |
...
Across all 11,591 BASKET(32) trades the model made $-1124.40 per share in total; the best three trades contributed $73.73 of that, leaving $-1198.13 for the other 11,588. **Read the middle trade beside the average.**

**How sure are we?**

- **BASKET(32).** Held-back window: 2,550 trades averaging -0.163R, middle trade -0.337R, honest range -0.213R to -0.113R. In-sample: 9,041 trades averaging -0.211R, middle trade -1.011R, range -0.248R to -0.174R.
...
- **Cost drag, which ENGINE-4 established is `cost per share ÷ stop distance`.** This variant's realised drag is 0.162R (95%: 0.141 to 0.184) — 16.2% of the money risked on every trade. The stop width sets that hurdle, not the instrument. See the stop-reading section for both readings side by side.
...
|---|---|---|---|---|---|---|---|
| BASKET(32) | 11,571 | 0.335% | 63.0¢ | 111.1¢ | $260 | **0.162R** | 0.141 to 0.184 |
...
| G2 | expectancy after costs (mean net R) | IS>=+0.10, OOS>=+0.05 | IS=-0.211, OOS=-0.163 | **FAIL** |
| G3 | profit factor after costs | IS>=1.20, OOS>=1.10 | IS=0.66, OOS=0.71 | **FAIL** |
| G4 | winners first going >=0.75R against | <=40% | 12.0% | **PASS** |
| G5 | mean net R > 0 in both regimes (in-sample) | both > 0 | bear (SPY < 50dma)=-0.128 (n=1510), bull (SPY > 50dma)=-0.228 (n=6749) | **FAIL** |

## BASKET(32), in full

| slice | n | hit | mean R | median R | mean % | payoff | PF | total R | maxDD R | max losing run |
|---|---|---|---|---|---|---|---|---|---|---|
| all | 11591 | 44.8% | -0.201 | -1.008 | -0.037% | 0.83 | 0.67 | -2324.5 | 2333.4 | 13 |
| in-sample 2023-09-01..2025-12-31 | 9041 | 44.3% | -0.211 | -1.011 | -0.037% | 0.83 | 0.66 | -1908.2 | 1913.1 | 13 |
| out-of-sample 2026-01-01..2026-08-28 | 2550 | 46.7% | -0.163 | -0.337 | -0.037% | 0.81 | 0.71 | -416.3 | 435.8 | 10 |
...
- MAE deciles (R): 0.10 | 0.22 | 0.39 | 0.65 | 1.00 | 1.02 | 1.07 | 1.14 | 1.30
- all trades reaching that far against: >=0.25R 77.7% · >=0.5R 65.4% · >=0.75R 57.0% · >=1.0R 50.3%
- **winners** that first went that far against: >=0.25R 54.2% · >=0.5R 28.4% · >=0.75R 11.8% · >=1.0R 0.0%
...
- MAE deciles (R): 0.09 | 0.20 | 0.36 | 0.60 | 0.96 | 1.02 | 1.06 | 1.13 | 1.29
- all trades reaching that far against: >=0.25R 76.3% · >=0.5R 64.4% · >=0.75R 55.4% · >=1.0R 48.6%
- **winners** that first went that far against: >=0.25R 52.7% · >=0.5R 28.5% · >=0.75R 11.1% · >=1.0R 0.0%

By regime (in-sample), then side and year:

| slice | n | hit | mean R | median R | mean % | payoff | PF | total R | maxDD R | max losing run |
|---|---|---|---|---|---|---|---|---|---|---|
| bear (SPY < 50dma) | 1510 | 47.7% | -0.128 | -0.110 | -0.026% | 0.84 | 0.77 | -194.0 | 205.6 | 10 |
| bull (SPY > 50dma) | 6749 | 43.2% | -0.228 | -1.014 | -0.038% | 0.85 | 0.64 | -1539.8 | 1547.8 | 13 |
| long | 6291 | 43.6% | -0.193 | -1.012 | -0.035% | 0.89 | 0.69 | -1216.4 | 1245.3 | 25 |
| short | 5300 | 46.3% | -0.209 | -0.520 | -0.040% | 0.75 | 0.65 | -1108.1 | 1115.5 | 12 |
| 2023 | 1307 | 45.0% | -0.233 | -1.014 | -0.039% | 0.78 | 0.64 | -304.2 | 310.3 | 14 |
| 2024 | 3861 | 43.8% | -0.187 | -1.014 | -0.025% | 0.89 | 0.70 | -723.2 | 734.6 | 11 |
| 2025 | 3873 | 44.5% | -0.227 | -1.008 | -0.049% | 0.79 | 0.63 | -880.8 | 887.8 | 13 |
| 2026 | 2550 | 46.7% | -0.163 | -0.337 | -0.037% | 0.81 | 0.71 | -416.3 | 435.8 | 10 |

- exits: {'target': 4446, 'stop': 5840, 'time': 1305}
- trades resolved by the pessimistic same-bar assumption: 19 (0.2%)
- mean 1-minute bars held: 53.3
...
| `bars_evaluated` | 1,586,229 |
| `bars_no_opening_range` | 0 |
| `bars_no_htf_trend` | 781,822 |
| `bars_no_break_on_trend_side` | 586,297 |
| `bars_direction_already_traded` | 205,585 |
