# Pre-registered gate — `orb_simple_1h.v1`
...
- **Trigger** a 5-minute candle CLOSING above the high (long) or below the low
  (short).
- **Filter** the 1-hour chart is in a confirmed trend, in that direction.
...
## The 1-hour convention, reused verbatim from ENGINE-3
...
* The day's final bucket is short and still counts. Seven bars a session — 09:30, 10:30 … 15:30, the last of them 30 minutes long.
...
The 1-hour reading CAN change during the day, five times: at 10:30, 11:30, 12:30, 13:30 and 14:30 a new hourly bar closes and the structure is re-read. Trend is therefore re-checked at every candidate 5-minute close, not judged once in the morning. A day can legitimately produce a long in the morning under an up reading and nothing in the afternoon because the reading went sideways.

The 1-hour chart turns faster than the 4-hour one, so this variant should trade MORE days and be more reactive to intraday reversals. If it produces fewer signals than the 4-hour variant, something is wrong with the implementation, not with the market.
