 ...
 *
 * The selector answer is the only state this site produces, and it has to
 * survive the jump into the app so onboarding's "Where are you right now?"
 * is already answered.
 *
 * Wire format:  ?path=learn | swing | pro
 *
...
export const START_ANSWER_FOR: Record<PathId, 'brand_new' | 'swing' | 'active'> = {
  learn: 'brand_new',
  swing: 'swing',
  pro: 'active',
};
 ...
 */
export const PLACEMENT_FOR: Record<PathId, { stage: string; mode: string; room: string }> = {
...
export function getAppHref(path: PathId): string {
  return `/get-the-app?path=${path}`;
}
