/**
 * POST   /api/v1/community/posts/:id/like — turn it on (idempotent)
 * DELETE /api/v1/community/posts/:id/like — turn it off (idempotent)
 *
...
import { setLike } from '@/lib/social/post-actions';

export const dynamic = 'force-dynamic';

async function run(ctx: Ctx & { params: { id: string } }, on: boolean): Promise<Response> {
  const r = await setLike({ postId: ctx.params.id, userId: ctx.user.id, on });
