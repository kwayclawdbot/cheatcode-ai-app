  ...
  return (
    <Screen variant="corner" layout="tab" testID="screen-paper">
      ...
      >
        <ObjectCard r={radius.xxl} style={{ padding: 18, gap: 6 }} testID="paper-balance">
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 7 }}>
            <T variant="meta" c={color.muted}>Practice balance</T>
            <View style={{ paddingHorizontal: 6, paddingVertical: 1, borderRadius: 4, borderWidth: 0.5, borderColor: alpha.cyan40 }}>
              <T variant="meta" weight="bold" c={color.cyan}>PAPER</T>
            </View>
          </View>
          <Num variant="keyPrice" weight="semibold">{usd(paper?.equity)}</Num>
          <T variant="meta" c={color.muted}>Nothing here is real money and nothing here reaches a broker.</T>
        </ObjectCard>

        <RowList>
          <Row>
            <T variant="meta" c={color.muted} style={{ flex: 1 }}>Cash</T>
            <Num variant="meta">{usd(paper?.cash)}</Num>
          </Row>
          <Row>
            <T variant="meta" c={color.muted} style={{ flex: 1 }}>Buying power</T>
            <Num variant="meta">{usd(paper?.buying_power)}</Num>
          </Row>
          <Row>
            <T variant="meta" c={color.muted} style={{ flex: 1 }}>Started at</T>
            <Num variant="meta">{usd(paper?.starting_balance)}</Num>
          </Row>
          <Row last>
            <T variant="meta" c={color.muted} style={{ flex: 1 }}>Resets used</T>
            <Num variant="meta">{paper?.reset_count ?? 0}</Num>
          </Row>
        </RowList>
        ...
        <ObjectCard r={radius.xl} style={{ padding: 15, gap: 9 }}>
          ...
          {alreadyResetThisMonth ? (
            <T variant="meta" c={color.gold}>{`Already reset in ${thisMonth}. The next one is available next month.`}</T>
          ) : null}
          <Button
        ...
        <T variant="meta" lh={16} c={color.dim} style={{ marginTop: 4 }} testID="paper-not-advice">
          {NOT_ADVICE_PAPER}
        </T>

        {notAvailable ? <NotConnected what="Your paper account" /> : error ? <T variant="meta" c={color.muted} align="center">{error}</T> : null}
        {isFixture ? <T variant="meta" c={color.dim} align="center">Sample account — the service is not connected here.</T> : null}
      </ScrollView>
      ...
      <Sheet visible={!!result} onClose={() => setResult(null)} title="Paper account">
        <T variant="meta" lh={20} c={color.muted}>{result}</T>
