...
function Line({ children }: { children: React.ReactNode }) {
  return (
    <View style={{ flexDirection: 'row', gap: 11, alignItems: 'flex-start' }}>
      ...
      <T variant="body" lh={21} style={{ flex: 1 }}>{children}</T>
    </View>
  );
}
  ...
  return (
    <Screen variant="dome" layout="stack" testID="screen-kai-plan">
      ...
      <View style={{ alignItems: 'center', gap: 14 }}>
        <KaiOrb size={62} />
        <T variant="sectionTitle" weight="bold" align="center" lh={29}>{'Here\'s how I\'ll\nwork for you'}</T>
      </View>

      <ScrollView style={{ flex: 1, marginTop: 22 }} contentContainerStyle={{ gap: 13, paddingBottom: 8 }} showsVerticalScrollIndicator={false}>
        <Line>
          I grade <T variant="body" lh={21} weight="bold">{MODE_LABEL[mode]}</T> setups across {focusShort} before you look.
        </Line>
        <Line>
          Every idea leads with what it risks — <T variant="body" lh={21} weight="bold">{usd(cap)}</T> is your daily ceiling.
        </Line>
        <Line>{EXPERIENCE_VOICE[guidance]}</Line>
        <Line>
          Nothing reaches a broker <T variant="body" lh={21} weight="bold">without your confirmation</T>.
        </Line>
        ...
        >
          <Row label="Brokerage">
            ...
            <T variant="meta" weight="semibold" c={color.muted} testID="plan-broker">
          ...
          <Row label="Practice mode">
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              <T variant="meta" c={color.cyan} testID="plan-paper">{paper.action}</T>
            </View>
          </Row>
          <Row label="Daily loss cap">
            <T variant="meta" weight="semibold" c={color.gold}>{usd(cap)}</T>
          </Row>
          <Row label="Morning briefing">
            <T variant="meta" weight="semibold">9:15 AM ET</T>
          </Row>
        </View>

        {draft.risk_answer === null ? (
          <T variant="meta" lh={17} c={color.dim} testID="plan-risk-note">{RISK_DEFAULT_NOTE}</T>
        ) : null}
        ...
        <View style={{ marginTop: 10, gap: 9 }} testID="plan-capabilities">
          <T variant="meta" weight="bold" c={color.muted}>Waiting for you</T>
              ...
              <View key={id} style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 9 }} testID={`plan-cap-${id}`}>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <T variant="body" weight="semibold" c={planned ? color.muted : color.text}>{c.title}</T>
                    {planned ? <Tag label={PLANNED_LABEL} c={color.muted} border={alpha.ivory20} /> : null}
                  </View>
                  <T variant="meta" lh={17} c={color.dim} style={{ marginTop: 1 }}>{c.plain}</T>
                </View>
              </View>
            );
          })}
        ...
        {error ? <T variant="meta" c={color.red} align="center">{error}</T> : null}
      </ScrollView>
