...
export function fixtureWorkspaceTurn(text: string): { reply: string; action: KaiWorkspaceAction } | null {
  ...
  if (/\bchart\b/.test(t)) return { reply: `Here is ${tick} on the daily. The shaded band is where it has been buying back in, and the line above it is yesterday's high — that is the level I would watch today.`, action: { type: 'open_chart', symbol: tick, timeframe: null, setup_id: null } };
